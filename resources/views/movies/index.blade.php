            ...
            <div class="row">
              ...
              <div class="col-md-3 mb-4">
                <div class="card h-100">
                  ...
                  @endphp
                  <img src="{{ $posterUrl }}" class="card-img-top" alt="{{ $movie['Title'] }}" style="height: 350px; object-fit: cover;">
                  <div class="card-body">
                    <h5 class="card-title">{{ $movie['Title'] }}</h5>
                    <p><strong>{{ __('dashboard.year') }}:</strong> {{ $movie['Year'] }}</p>
                    <a href="{{ route('movies.detail', $movie['imdbID']) }}" class="btn btn-primary btn-sm btn-block">{{ __('dashboard.detail') }}</a>
                    <form method="POST" action="{{ route('movies.store_favorite') }}" class="mt-2">
                      ...
                      <button class="btn btn-danger btn-sm btn-block">{{ __('dashboard.add_to_favorite') }}</button>
                    </form>
                  </div>
                </div>
              </div>

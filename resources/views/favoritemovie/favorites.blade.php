          ...
          <div class="row">
            ...
            <div class="col-md-3 mb-4">
              <div class="card h-100">
                ...
                @endphp
                <img src="{{ $posterUrl }}" class="card-img-top" alt="{{ $fav->title }}" style="height: 350px; object-fit: cover;">
                <div class="card-body">
                  <h5 class="card-title">{{ $fav->title }}</h5>
                  <p><strong>{{ __('dashboard.year') }}:</strong> {{ $fav->year ?? 'N/A' }}</p>
                  <a href="{{ route('movies.detail', $fav->imdb_id) }}" class="btn btn-info btn-sm btn-block">{{ __('dashboard.detail') }}</a>
                  <form method="POST" action="{{ route('favorites.destroy', $fav->imdb_id) }}" class="mt-2">
                    @csrf
                    @method('DELETE')
                    <button class="btn btn-danger btn-sm btn-block">{{ __('dashboard.remove') }}</button>
                  </form>
                </div>
              </div>
            </div>
